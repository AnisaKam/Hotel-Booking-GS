...
const BookIcon = () => (
   ...
   fill="none" viewBox="0 0 24 24" ><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 19V4a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v13H7a2 2 0 0 0-2 2Zm0 0a2 2 0 0 0 2 2h12M9 3v14m7 0v4" />
   </svg> 
)
    ...
    return (
            <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                <div className="hidden md:flex items-center gap-4">
                    ...
                    {user ? 
                    (<UserButton>             {/*код для кнопки в правом верхнем углу Пользователя (моя)*/}
                        <UserButton.MenuItems>
                           <UserButton.Action label="My Bookings" labelIcon={<BookIcon/>} onClick={() => navigate('/my-bookings') }/> 
                        </UserButton.MenuItems>
                    </UserButton>)
                    :
                    (<button onClick={openSignIn} className={`px-8 py-2.5 rounded-full ml-8 transition-all duration-500 ${isScrolled ? "text-black bg-white" : "bg-black text-white"}`}>
                        Login
                    </button>)
                    }
